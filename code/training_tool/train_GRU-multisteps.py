...
if __name__ == "__main__":
    ...
    model = CustomModel("GRU-multisteps",
                        dataset)

    model.train(epochs=500, batch_size=64, patience=30, scheduler=my_scheduler)
    
    with open(f'../trained_models/EIA2022_CERTH_{dataset_name}_GRU-multisteps_{training_time}_{period_time}_loss.json', 'w') as f:
        json.dump(model.log, f, indent=4)
        f.close()
        
    model.model.save(f"../trained_models/EIA2022_CERTH_{dataset_name}_GRU-multisteps_{training_time}_{period_time}.h5")