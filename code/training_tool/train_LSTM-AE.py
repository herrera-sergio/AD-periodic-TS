...
if __name__ == "__main__":
    ...
    print(dataset.X_train.shape, dataset.X_val.shape)

    model = CustomModel("LSTM-Autoencoder",
                        dataset)

    model.train(epochs=500, batch_size=64, patience=30, scheduler=my_scheduler)
    
    with open(f'../trained_models/EIA2022_CERTH_{dataset_name}_LSTM-AE_{training_time}_{period_time}_loss.json', 'w') as f:
        json.dump(model.log, f, indent=4)
        f.close()
        
    model.model.save(f"../trained_models/EIA2022_CERTH_{dataset_name}_LSTM-AE_{training_time}_{period_time}.h5")